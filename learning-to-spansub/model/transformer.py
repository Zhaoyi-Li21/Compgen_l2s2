...
def _run_encoder(src, src_lengths, model):
...
def _translate_batch_with_strategy(
    model, src, src_lengths, decode_strategy
):
    ...
    src, enc_states, memory_bank, src_lengths = _run_encoder(src, src_lengths, model)
...
class TransformerEncDec(nn.Module):
    ...
    def forward(self, inp, out, lens=None, batch_sum=True):
        #print('inp:', inp.shape, inp[:,1])
        #print('out:', out.shape, out[:,1])
        #print('lens:', lens.shape, lens[1])
        model_outputs = self.model(inp.unsqueeze(-1), out.unsqueeze(-1), lens)
    ...
    def sample(
        ...
        sample_results = _translate_batch_with_strategy(self.model, inp.unsqueeze(-1), lens, decode_strategy)
